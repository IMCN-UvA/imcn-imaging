...
/**
 *
 *  This algorithm uses the classical GDM framework to smooth a single object
...
public class SmoothGdm {
	...
	private		boolean			mapIntensity = false;
	 ...
	 */
	public SmoothGdm(float[] lvlsetin_, int nx_, int ny_, int nz_, float rx_, float ry_, float rz_,
	...
	public final void addIntensityToMap(float[] intens) {
		intensity = intens;
		mapIntensity = true;
	}
	public final void changeNarrowBandSize(float newsize) {
		landmineDist = newsize;
	...
	public final float[] exportLevelset() {
		float[] res = new float[nx*ny*nz];
		for (int xyz = 0; xyz<nx*ny*nz; xyz++) res[xyz] = levelset[xyz];
		return res;
	}
	
 	public final float[][][] exportLevelset3d() {
		float[][][] res = new float[nx][ny][nz];
		for (int x=0;x<nx;x++) for (int y=0;y<ny;y++) for (int z=0;z<nz;z++) {
	...
	public final float[] exportIntensity() {
		float[] res = new float[nx*ny*nz];
		for (int xyz = 0; xyz<nx*ny*nz; xyz++) res[xyz] = intensity[xyz];
		return res;
	}
     ...
     public final void fastMarchingReinitialization(boolean narrowBandOnly) {
		...
		for (int xyz = 0; xyz<nx*ny*nz; xyz++) if (!mask[xyz]) {
			// check for inside vs. outside on the original levelset (assumed to be unchanged away from boundary)
			if (inlevelset[xyz]<0)	levelset[xyz] = -maxdist;
    ...
    *	(the reinitialization is incorporated)
    */
    public final void evolveNarrowBandMapping(int iter, float mindiff) {
    	...
    	double delta;
    	float[] mapped = new float[nx*ny*nz];
		...
		for (int t=0;t<iter && diff>mindiff;t++) {
			...
			for (int n=0; n<narrowband.currentsize;n++) {
				...
				// compose intensity
				mapped[xyz] = ImageInterpolation.linearClosestInterpolation(intensity, (float)pt[X], (float)pt[Y], (float)pt[Z], nx, ny, nz);
			}
			diff = (nswap/(float)boundarysize);
			if (debug) System.out.print("changed labels: "+nswap+" ("+(diff*100.0f)+" % of boundary)\n");
    ...
    *	(the reinitialization is incorporated)
    */
    public final void evolveCompleteMapping(int iter, float mindiff) {
    	if (debug) System.out.print("level set evolution: complete image + mapping\n");
        ...
        */
        int boundarysize=0;
		float[] nextls = new float[nx*ny*nz];
		for (int xyz = 0; xyz<nx*ny*nz; xyz++) if (mask[xyz]) {
			nextls[xyz] = levelset[xyz];
			if (Numerics.abs(levelset[xyz])<1.0 && levelset[xyz]!=UNKNOWN) boundarysize++;
			/*
			// the criterion for being in the narrow band is to have a short distance to closest boundaries
			if (Numerics.abs(levelset[xyz])<narrowBandDist) {
    ...
    private final double levelsetForces(int xyz) {
		...
		balloonforce = -Numerics.bounded( inlevelset[xyz] - levelset[xyz], -1.0, 1.0);

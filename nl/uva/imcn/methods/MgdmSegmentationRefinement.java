...
public class MgdmSegmentationRefinement {
	
	
	// image data
	private 	float[][]		edges;  			// original image edges
	...
	private 	float[][] 		initfunctions;  	// MGDM's pseudo level set mgdmfunctions
	private 	byte[][] 		initlabels;   		// MGDM's label maps
	private 	float[][] 		priorfunctions;  	// intensity-based priors (best values)
	private 	byte[][] 		priorlabels;   		// intensity-based priors (best labels)
	...
	// parameters
	private	double		smoothweight, forceweight, edgeweight, priorweight;
	private	double		spatialdist, spatialvar;
	...
	public MgdmSegmentationRefinement(float[] img_, boolean[] mask_,
									...
									int nmgdm_, float fw_, float sw_, float pw_, float ew_, float sd_,
									String priorType_, boolean skipBg_,
		...
		if (verbose) System.out.print("building edge map\n");
		edges = computeEdgeMap(img_, mask, sd_);
		
		nmgdm = (byte)nmgdm_;
						
		if (verbose) System.out.print("building prior maps\n");
		computePriorMaps(img_, mask, init_, priorType_, skipBg_);
		...
		spatialdist = 3.0f*sd_;
		spatialvar = 9.0f*spatialdist*spatialdist;
		
		if (verbose) System.out.print("MGDM parameters: "+fw_+" (force), "+sw_+" (smoothing), "+ew_+" (edges)\n");
		...
		try {
			...
			initfunctions = new float[nmgdm][nix*niy*niz];
			initlabels = new byte[nmgdm+1][nix*niy*niz];	
		...
		fastMarchingInitialization(false, false);
		
		// copy into init MGDM
		for (int xyz=0; xyz<nix*niy*niz; xyz++) {
			for (int n=0;n<nmgdm;n++) {
				initfunctions[n][xyz] = mgdmfunctions[n][xyz];	
				initlabels[n][xyz] = mgdmlabels[n][xyz];	
			}
     ...
     *	(smoothing scale is in voxels) */
    public final float[][] computeEdgeMap(float[] img, boolean[] mask, float smoothingScale) {
    	
    	if (debug) System.out.print("smoothing... ");
    	float[][] kernel = ImageFilters.separableGaussianKernel(smoothingScale, smoothingScale, smoothingScale);
    	// the image is not reusable!!
    	img = ImageFilters.separableConvolution(img,nix,niy,niz,kernel);
    	
    	if (debug) System.out.print("scale... ");
    	double gradientScale = 0.0;
    	double den = 0.0;
    	for (int xyz=0; xyz<nix*niy*niz; xyz++) {
    		if (mask[xyz]) {
				gradientScale += Numerics.square(0.5*(img[xyz+1]-img[xyz-1]));
				gradientScale += Numerics.square(0.5*(img[xyz+nix]-img[xyz-nix]));
				gradientScale += Numerics.square(0.5*(img[xyz+nix*niy]-img[xyz-nix*niy]));
				den+=3;
			}
		}
		gradientScale = FastMath.sqrt( gradientScale/(den-1.0) );
		if (debug) System.out.print("("+gradientScale+")");
    	
    	if (debug) System.out.print("edges... ");
    	float[] edgemap = new float[nix*niy*niz];
    	double dsx, dsy, dsz;
    	for (int xyz=0; xyz<nix*niy*niz; xyz++) {
    		if (mask[xyz]) {
				dsx = 0.5*(img[xyz+1]-img[xyz-1])/gradientScale;
				dsy = 0.5*(img[xyz+nix]-img[xyz-nix])/gradientScale;
				dsz = 0.5*(img[xyz+nix*niy]-img[xyz-nix*niy])/gradientScale;
				
				edgemap[xyz] = (float)(1.0/(1.0 + dsx*dsx + dsy*dsy + dsz*dsz));
			} else {
				edgemap[xyz] = 1.0f;
			}
		}
    	...
    	if (debug) System.out.print("gradient... ");
    	float[][] edgegrad = new float[4][nix*niy*niz];
    	for (int xyz=0; xyz<nix*niy*niz; xyz++) if (mask[xyz]) {
    		edgegrad[X][xyz] = -(edgemap[xyz+1]-edgemap[xyz-1]);
    		edgegrad[Y][xyz] = -(edgemap[xyz+nix]-edgemap[xyz-nix]);
    		edgegrad[Z][xyz] = -(edgemap[xyz+nix*niy]-edgemap[xyz-nix*niy]);
    		/*
    		edgegrad[pX][xyz] = -(edgemap[xyz+1]-edgemap[xyz]);
    		edgegrad[pY][xyz] = -(edgemap[xyz+nix]-edgemap[xyz]);
    		edgegrad[pZ][xyz] = -(edgemap[xyz+nix*niy]-edgemap[xyz]);
    		
    		edgegrad[mX][xyz] = -(edgemap[xyz]-edgemap[xyz-1]);
    		edgegrad[mY][xyz] = -(edgemap[xyz]-edgemap[xyz-nix]);
    		edgegrad[mZ][xyz] = -(edgemap[xyz]-edgemap[xyz-nix*niy]);
    		*/
    		// compute norm and normalize directions
    		edgegrad[N][xyz] = (float)FastMath.sqrt(Numerics.square(edgegrad[X][xyz]+edgegrad[X][xyz])
    										 		+Numerics.square(edgegrad[Y][xyz]+edgegrad[Y][xyz])
													+Numerics.square(edgegrad[Z][xyz]+edgegrad[Z][xyz]));
			/*
			if (edgegrad[N][xyz]>0.01) {
				edgegrad[pX][xyz] /= edgegrad[N][xyz];
				edgegrad[pY][xyz] /= edgegrad[N][xyz];
				edgegrad[pZ][xyz] /= edgegrad[N][xyz];
				edgegrad[mX][xyz] /= edgegrad[N][xyz];
				edgegrad[mY][xyz] /= edgegrad[N][xyz];
				edgegrad[mZ][xyz] /= edgegrad[N][xyz];
			}
			*/
    	}
   ...
   /** precompute the edge map and its gradient based on a smoothed version of the image
     *	(smoothing scale is in voxels) */
    public final void computePriorMaps(float[] img, boolean[] mask, byte[] initseg, String type, boolean skipBg) {
    	
    	// find number of labels
    	byte[] lblist = ObjectLabeling.listOrderedLabels(initseg, nix, niy, niz);
    	int nlb = lblist.length;
    	
    	// build stats
    	float[] mean = new float[nlb];
    	float[] stdev = new float[nlb];
    	float[] sum = new float[nlb];
    	for (int n=0;n<nlb;n++) {
    		mean[n] = 0.0f;
    		stdev[n] = 0.0f;
    		sum[n] = 0.0f;
		...
		for (int xyz=0; xyz<nix*niy*niz; xyz++) {
			if (mask[xyz]) {
				for (int n=0;n<nlb;n++) if (initseg[xyz]==lblist[n]) {
					mean[n] += img[xyz];
					sum[n] += 1.0f;
				}
			}
		}
		for (int n=0;n<nlb;n++) if (sum[n]>0) mean[n] /= sum[n];
		
		for (int xyz=0; xyz<nix*niy*niz; xyz++) {
			if (mask[xyz]) {
				for (int n=0;n<nlb;n++) if (initseg[xyz]==lblist[n]) {
					stdev[n] += Numerics.square(img[xyz]-mean[n]);
				}
			}
		}
		for (int n=0;n<nlb;n++) if (sum[n]>0) stdev[n] = (float)FastMath.sqrt(stdev[n]/sum[n]);
		
    	// here we would do the fancier robust of FCM-based stuff (so far: simple Gaussian)
    	if (type.equals("robust")) {
			// generate data arrays per label
			int n0=0; 
			if (skipBg) n0=1;
			for (int n=n0;n<nlb;n++) {
				double[] sample = new double[(int)sum[n]];
				int id=0;
				for (int xyz=0; xyz<nix*niy*niz; xyz++) {
					if (mask[xyz]) {
						if (initseg[xyz]==lblist[n]) {
							sample[id] = img[xyz];
							id++;
						}
					}
				}
				Percentile score = new Percentile();
				score.setData(sample);
				double med = score.evaluate(50);
				double low = score.evaluate(15.9);
				double high = score.evaluate(84.1);
				System.out.print("robust estimates: lb "+n+" ("+lblist[n]+"): "+med+" ["+low+" , "+high+"]\n");
				mean[n] = (float)med;
				stdev[n] = (float)(0.5*(high-low));
			}
		}
		
		if (debug) {
			System.out.print("intensity priors: \n");
			for (int n=0;n<nlb;n++) System.out.print("lb "+n+" ("+lblist[n]+"): "+mean[n]+" +/- "+stdev[n]+"\n");
		}
    	
		// init the maps : same number as MGDM fields
    	priorfunctions = new float[nmgdm][nix*niy*niz];
    	priorlabels = new byte[nmgdm][nix*niy*niz];
    	
    	double[] proba = new double[nlb];
    	double[] bestval = new double[nmgdm];
    	byte[] bestid = new byte[nmgdm];
    	int n0=0;
    	if (skipBg) n0=1;
    	double maxproba;
    	for (int xyz=0; xyz<nix*niy*niz; xyz++) {
    		if (mask[xyz]) {
    			maxproba = 0.0;
    			for (int n=n0;n<nlb;n++) if (stdev[n]>0) {
    				proba[n] = FastMath.exp(-0.5*Numerics.square((img[xyz]-mean[n])/stdev[n]));
    				if (proba[n]>maxproba) maxproba = proba[n];
    			}
    			if (skipBg) proba[0] = 1.0 - maxproba;
    			
    			Numerics.argmax(bestid, bestval, proba, nmgdm);
    			for (int m=0;m<nmgdm;m++) {
    				priorfunctions[m][xyz] = (float)bestval[m];
    				priorlabels[m][xyz] = bestid[m];
    			}
    		}
    	}
    ...
    public final float[][][] exportEdgeNorm() {
    	float[][][] result = new float[nix][niy][niz];
    	
    	for (int x=0;x<nix;x++) for (int y=0;y<niy;y++) for (int z=0;z<niz;z++) {
    		/*
    		result[x][y][z] = (float)FastMath.sqrt(edges[X][x+nix*y+nix*niy*z]*edges[X][x+nix*y+nix*niy*z]
    												+edges[Y][x+nix*y+nix*niy*z]*edges[Y][x+nix*y+nix*niy*z]
    												+edges[Z][x+nix*y+nix*niy*z]*edges[Z][x+nix*y+nix*niy*z]);
    												*/
    		result[x][y][z] = edges[N][x+nix*y+nix*niy*z];   												
		}
		return result;
    }
    ...
    public final float[][][][] exportPriorFunctions() {
    	float[][][][] result = new float[nix][niy][niz][nmgdm];
    	
    	for (int x=0;x<nix;x++) for (int y=0;y<niy;y++) for (int z=0;z<niz;z++) if (mask[x+nix*y+nix*niy*z]) {
    		for (int n=0;n<nmgdm;n++) result[x][y][z][n] = priorfunctions[n][x+nix*y+nix*niy*z];
		}
		return result;
    }
    public final byte[][][][] exportPriorLabels() {
    	byte[][][][] result = new byte[nix][niy][niz][nmgdm];
    	
    	for (int x=0;x<nix;x++) for (int y=0;y<niy;y++) for (int z=0;z<niz;z++) if (mask[x+nix*y+nix*niy*z]) {
    		for (int n=0;n<nmgdm;n++) result[x][y][z][n] = priorlabels[n][x+nix*y+nix*niy*z];
		}
		return result;
    }
	/**
    ...
    */
    public final void evolveNarrowBand(int iter, float mindiff) {
		...
		// init decomposition: doen in the first step
 	...
 	private final void levelsetForces(int xyz, double[] forces) {
		...
		for (int n=0;n<=nmgdm;n++) {
			...
			// fast: reconstruct original distance and check if close enough
			double phi0;
			if (n==0) phi0 = -initfunctions[0][xyz];
			else  phi0 = 0.0;
			for (int l=0;l<n;l++) {
				phi0 += initfunctions[l][xyz];
			}
			...
			distval[n] = 1.0/(1.0+Numerics.square(phi[CTR]/spatialdist));
			if (distval[n]<0.001) forces[n] = 0.0;
			else {
				...
				// regularization
				forces[n] = -distval[n]*smoothweight*stepsize*K*GPhi;

				// edge forces
				forces[n] += distval[n]*edgeweight*stepsize
							*(Numerics.max(edges[X][xyz],0)*Dmx[n] + Numerics.max(edges[X][xyz],0)*Dpx[n] 
							 +Numerics.max(edges[Y][xyz],0)*Dmy[n] + Numerics.max(edges[Y][xyz],0)*Dpy[n] 
							 +Numerics.max(edges[Z][xyz],0)*Dmz[n] + Numerics.max(edges[Z][xyz],0)*Dpz[n]);
				
				// original distance balloon forces
				forces[n] += distval[n]*forceweight*stepsize*(phi[CTR]-phi0)*GPhi;
				
				// prior based forces: just balloon forces? no, use the full MGDM approach!!
			}
		}
		
		// find the best target in the neighborhood to build the corresponding force
		// (product of gain and distval => must check all)
		bestlb = EMPTY;
		bestval = -1.0f;
		done=false;
		for (int n=0;n<nmgdm && !done;n++) {
			gainlb = priorlabels[n][xyz];
			gainval = priorfunctions[n][xyz];
		...
		if (bestlb!=EMPTY) {
			for (int n=0;n<=nmgdm;n++) {
				...
				if (lb!=EMPTY) {
					// upwind scheme
					forces[n] += priorweight*stepsize*bestval
